    ...
    function synchronizeCalendar($calendar_id)
    {
        ...
        foreach ($calLinks as $calLink) {
            ...
            foreach ($events as $event) {
                ...
                $dateEnd = (isset($event->dtend)) ? $event->dtend : $event['DTEND'];
				$summary = (isset($event->summary)) ? $event->summary : $event['SUMMARY'];
				$dateStart = (isset($event->dtstart)) ? $event->dtstart : $event['DTSTART'];

                $summary_rpl = preg_replace('/[\x00-\x1F\x80-\xFF]/', '', $summary);

                $booking_data = array(
                    'booking_calendar_id'=>$cal->calendar_id,
                    'booking_source'=>$calLink->url,
                    'booking_date_from'=> date('Y-m-d H:i:s', $ical->iCalDateToUnixTimestamp($dateStart)),
                    'booking_date_to'=> date('Y-m-d H:i:s', $ical->iCalDateToUnixTimestamp($dateEnd)),
                    'booking_guests'=>'',
                    'booking_user_fname'=>'',
                    'booking_user_lname'=>'',
                    'booking_user_email'=>'',
                    'booking_summary'=> ( $summary == 'UNAVAILABLE') ? "None" : $summary_rpl,
