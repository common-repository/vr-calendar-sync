...
class VRCalendarAdmin extends VRCSingleton
{
    ...
    {
        $msg = esc_html( __('Something went wrong!', VRCALENDAR_PLUGIN_TEXT_DOMAIN) );
        $type = esc_html( __('error', VRCALENDAR_PLUGIN_TEXT_DOMAIN) );

        $save = true;
        if (isset($_GET['cal_id'])) {
            $calid = sanitize_text_field($_GET['cal_id']);
            $VRCalendarEntity = VRCalendarEntity::getInstance();
            $sync = $VRCalendarEntity->synchronizeCalendar($calid);

            if($sync->linesStatus === "false"){
                $filename = $sync->linesFilename;
                $save = false;
            }
        }

        if($save){
            $msg = esc_html( __('Calendar Synchronized successfully', VRCALENDAR_PLUGIN_TEXT_DOMAIN) );
            $type = esc_html( __('updated', VRCALENDAR_PLUGIN_TEXT_DOMAIN) );
        }else{
            $msg = __("Something went wrong! Please kindly double check your {$filename} link.", VRCALENDAR_PLUGIN_TEXT_DOMAIN);
            $type = esc_html( __('error', VRCALENDAR_PLUGIN_TEXT_DOMAIN) );
        }
    ...
    function dashboard()
    {
        include VRCALENDAR_PLUGIN_DIR.'/Admin/Views/Dashboard.php';
    }
